...
class PaddedCollatorForActionPrediction_Nav_MMN:
    ...
    def __call__(self, instances: Sequence[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        ...
        # Stack c_image
        c_image = [torch.from_numpy(np.copy(instance["c_image"])) for instance in instances]
        c_image = torch.stack(c_image)
        
        # Stack p_image
        p_image = [torch.from_numpy(np.copy(instance["p_image"])) for instance in instances]
        p_image = torch.stack(p_image)        
